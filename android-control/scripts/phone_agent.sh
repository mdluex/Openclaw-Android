...
do_action() {
    ...
    case "$ATYPE" in
        tap)
            run_cmd "input tap $2 $3" 2>/dev/null
            echo "  👆 Tap ($2, $3)"
            ;;
        swipe)
            run_cmd "input swipe $2 $3 $4 $5 ${6:-300}" 2>/dev/null
            echo "  👆 Swipe ($2,$3)→($4,$5)"
            ;;
        type)
            local TEXT="$2"
            local ESCAPED=$(echo "$TEXT" | sed 's/ /%s/g')
            run_cmd "input text '$ESCAPED'" 2>/dev/null
            echo "  ⌨️ Type: $TEXT"
            ;;
        key)
            run_cmd "input keyevent $2" 2>/dev/null
            echo "  🔘 Key: $2"
            ;;
        open_app)
            run_cmd "monkey -p $2 -c android.intent.category.LAUNCHER 1" 2>/dev/null
            echo "  📱 Open: $2"
            sleep 2
            ;;
        scroll_down)
            run_cmd "input swipe 540 1600 540 600 300" 2>/dev/null
            echo "  ⬇️ Scroll down"
            ;;
        scroll_up)
            run_cmd "input swipe 540 600 540 1600 300" 2>/dev/null
            echo "  ⬆️ Scroll up"
            ;;
        go_home)
            run_cmd "input keyevent 3" 2>/dev/null
            echo "  🏠 Home"
            ;;
        go_back)
            run_cmd "input keyevent 4" 2>/dev/null
            echo "  ◀️ Back"
            ;;
        wait)
            echo "  ⏳ Wait ${2:-1}s"
...
for STEP in $(seq 1 $MAX_STEPS); do
    ...
    # 1. Screenshot
    echo "  📸 Capturing..."
    run_cmd "screencap -p $SCREENSHOT_PATH" 2>/dev/null
    sleep 0.5
    ...
    echo "  🔍 Reading UI..."
    run_cmd "uiautomator dump /sdcard/ui.xml" 2>/dev/null
    sleep 0.3
    UI_ELEMENTS=$(run_cmd "cat /sdcard/ui.xml" 2>/dev/null | grep -o 'text="[^"]*"[^>]*bounds="\[[0-9]*,[0-9]*\]\[[0-9]*,[0-9]*\]"' | head -20 | while read -r line; do
